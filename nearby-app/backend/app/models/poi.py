...
class PointOfInterest(Base):
    ...
    description_short = Column(Text)  # 250 visible char limit, validated in admin schema
    teaser_paragraph = Column(Text)
...
class Event(Base):
    ...
    venue_poi_id = Column(PG_UUID(as_uuid=True), nullable=True)
    venue_inheritance = Column(JSONB, nullable=True)
    # Recurring events (Task 50)
    series_id = Column(PG_UUID(as_uuid=True), nullable=True)
    parent_event_id = Column(PG_UUID(as_uuid=True), nullable=True)
    excluded_dates = Column(JSONB, nullable=True)
    recurrence_end_date = Column(TIMESTAMP(timezone=True), nullable=True)
    manual_dates = Column(JSONB, nullable=True)
